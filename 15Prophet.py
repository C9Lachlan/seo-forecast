...
test_data_end_date = st.sidebar.date_input("Test Data End Date")

# Initial forecast range days (could be user input)
initial_forecast_range_days = st.sidebar.number_input("Initial Forecast Range (days)", min_value=1, value=182, help="Might be adjusted depending on the test data.")
...
# Function to calculate and adjust forecast range days
def calculate_forecast_range(forecast_data, test_data, initial_range):
    if not test_data.empty:
        last_forecast_date = forecast_data['ds'].max()
        required_days = (test_data['ds'].max() - last_forecast_date).days
        return max(initial_range, required_days)
...
if uploaded_file is not None:
    ...
    test_data_end_date = pd.to_datetime(test_data_end_date)
    if test_data_end_date in df['ds'].values:
    ...
    # Calculate and display the effective forecast range
    forecast_range_days = calculate_forecast_range(forecast_data, test_data, initial_forecast_range_days)
    st.sidebar.write(f"Effective Forecast Range: {forecast_range_days} days")
    ...
    settings_columns = {
        ...
        "Forecast Range Days": forecast_range_days  # Updated dynamically if needed
    }
