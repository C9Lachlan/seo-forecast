...
if uploaded_file is not None:
    ...
    test_data = test_data.copy()  # Avoid the SettingWithCopyWarning
    ...
    actual_test_line = go.Scatter(
        ...
        name='Actual Test',
        marker=dict(
            color='salmon',  # Slightly darker color for actual test scatter
            size=2
        )
    )
    ...
    actual_forecast_line = go.Scatter(
        ...
        marker=dict(
            color='white',  # Lighter color for actual pre-forecast scatter
            size=2
        )
    )
