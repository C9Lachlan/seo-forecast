...
if uploaded_file is not None:
    ...
    actual_line = go.Scatter(
        ...
        marker=dict(
            color='salmon',
            size=2
            )  # Set scatter data color and size
    )

    # Update the figure with these lines
    fig.add_traces([forecast_line, actual_line])
    ...
    settings_columns = {
        ...
        "Algorithm Updates": alg_updates,
        "Referring Domains": referring_domains_check,
        "Outlier Adjustment": outlier_adjustment,
    ...
    st.table(pd.DataFrame(list(settings_columns.items()), columns=["Parameter", "Value"]))
